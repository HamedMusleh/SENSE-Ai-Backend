...
def process_child_audio(audio_path, conversation_history=None):
    ...
    # =========================
    # Build history for triage
    # =========================
    history_start = time.perf_counter()
    triage_history = _build_triage_history(conversation_history)
    _print_time("BUILD TRIAGE HISTORY", history_start)
    ...
    _print_time("TRIAGE", triage_start)

    # =========================
    # Append turn to history before reply
    # =========================
    turn_number = len(conversation_history) + 1
    ...
    _print_time("LLM REPLY", reply_start)

    reply_text = teta_output.get("reply_text", "")

    conversation_history[-1]["assistant_reply"] = reply_text
    conversation_history[-1]["response_source"] = teta_output.get("source", "openai")

    _print_time("TOTAL ORCHESTRATOR", total_start)

    return {
        "raw_text": raw_text,
        "processed_text": processed_text,

        "emotion_result": conversation_history[-1]["emotion_result"],
        "weighted_result": conversation_history[-1]["weighted_result"],
        "triage_result": triage_result,

        "reply_text": reply_text,

        "response_source": teta_output.get("source", "openai"),
        "response_strategy_label": teta_output.get("strategy_label"),
        "response_strategy_signal": teta_output.get("strategy_risk_signal"),
...
def end_session(conversation_history, session_id=None, print_report=True):
    ...
    _print_time("FINAL SESSION REPORT", report_start)
